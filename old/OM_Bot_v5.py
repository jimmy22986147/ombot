...
def variable_zone(env='test'):
    #gc = pygsheets.authorize(service_account_file='C://Users//user//Desktop//project//OMBOT//aicool-csombot-81676ba6309e.json')
    gc = pygsheets.authorize(service_account_file='C://Users//user//Desktop//project//OMBOT//aicool-csbot-0539c368976e.json')
    
    if env == 'prod':
        survey_url = 'https://docs.google.com/spreadsheets/d/1jZ7XEhS0IYUI6mkf_YOQefgUiYTVZYRU-93my3cZJgs/edit#gid=0'  
        table = 'cs_detection.tasks'
        token = '1918831254:AAExIEvOAeEUzb6Xmx2GrTb6HfwwkBJD7p4'
    elif env == 'test':
        survey_url = 'https://docs.google.com/spreadsheets/d/10dvYLkKzAW_6mJFGR2LB7Mx0WY_c_8GLtqhqen0q3hY/edit#gid=1481259803'
...
#GOOGLE SHEET變數, #Telegram Bot變數
env = 'prod'
ws, table, token, allow_list = variable_zone(env)
bot = telepot.Bot(token)
'''
text = '23 CHECK'
df = ws_to_df(ws)
s = time.time()
return_text = om_bot(text, df)
print(time.time()-s )
'''
...
def handle(msg):
    ...
    if chat_id_str in allow_list:
        try:
            #db
            mydb = mysql.connector.connect(host="172.16.150.100",#"localhost",
                                           user="csbotmgr",
                                           password="1q2w3e4r5t",
            ...
            if r == 'CHECK':
                ...
                bot.sendMessage(chat_id, '執行中，請稍等')
            return_text = om_bot(text, df)    
